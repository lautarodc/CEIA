...
from ExpectationMaximization import EMScalar
from SinteticDataset import SinteticDataset
from metrics import Accuracy, Precision, Recall
from kmeans_basic import k_means, k_means_classify
...
var = np.array([15, 2])
p = 0.25
dataset = SinteticDataset(mu, var, p, 500)
...
# K-means
print("K-means")
centers, ids = k_means(x_train, 2, 1000)
...
for i in range(list_var.shape[0]):
    ...
    mu = np.array([5, 10])
    # var = np.array([list_var[idx[2]], list_var[idx[3]]])
    var = np.array([15, list_var[i]])
    mu = mu.reshape(-1, 1)
    var = var.reshape(-1, 1)
    dataset = SinteticDataset(mu, var, 0.25, 500)
    x_train, x_test, y_train, y_test = dataset.split(0.8)
    x_train = x_train.reshape(-1, 1)
    x_test = x_test.reshape(-1, 1)
...
# plt.scatter(ovl[index], metric_ovl[index, 0])
